  ...
  return {
    ...
    renderCell: (cell) => {
      ...
      return (
        ...
        >
          {col.index == -1 ? <p className="text-slate-400 text-sm"> {rowIndex} </p>: formatContent(row[`col${col.index}`] || '')}
        </div>
      );
    },
  };
...
const ExcelDataViewer: React.FC<ExcelDataViewerProps> = ({fileUrl, status}) => {
  ...
  useEffect(() => {
    if (sheets.length > 0 && sheets[activeSheet]) {
      ...
      // Add an "index" column as the first column with a fixed width
      const columns = [
        // {
        //   key: indexColumn,
        //   name: '',
  ...
  const excelComponent = () => {
    if (sheets.length > 0 && columns.length > 0) {
      return (
        <div className="px-4 flex flex-col shadow-lg rounded-sm">
